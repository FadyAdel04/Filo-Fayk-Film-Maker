...
// Example testimonials (replace with your real clients if desired!)
const testimonials = [
  {
    name: "Alice Chen",
    ...
    <div className="section-container">
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {testimonials.map((testi, idx) => (
          <div
            key={idx}
            className="relative bg-card/80 rounded-2xl shadow-lg p-8 flex flex-col items-center text-center hover:scale-105 hover:shadow-xl transition-transform animate-fade-in"
            style={{
          ...
          >
            ...
            <div className="flex justify-center mb-2">
              {[...Array(5)].map((_, starIdx) => (
                <Star key={starIdx} size={20} className={`mx-0.5 ${starIdx < testi.rating ? 'text-yellow-400' : 'text-muted-foreground'}`} strokeWidth={2} fill={starIdx < testi.rating ? '#facc15' : 'none'} />
              ))}
            </div>
            <p className="italic text-lg text-foreground/90 mb-4">"{testi.feedback}"</p>
            <span className="font-semibold text-primary">{testi.name}</span>
            <span className="text-xs text-muted-foreground">{testi.title}</span>
